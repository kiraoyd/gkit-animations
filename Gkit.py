...
def G_polygon(x,y, n):
    w = FPTlib.Polygon_DX(x,y,  ctypes.c_double(n)) # Polygon_DX actually wants the number of items passed in as a double

def G_fill_polygon(x,y, n):
    w = FPTlib.Fill_Polygon_DX(x,y,  ctypes.c_double(n))
    
def G_py_polygon(x,y):
    lx = len(x)
    ly = len(y)
    if lx < ly :
        n = lx
